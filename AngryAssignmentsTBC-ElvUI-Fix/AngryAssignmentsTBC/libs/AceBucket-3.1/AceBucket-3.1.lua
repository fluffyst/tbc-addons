--[[ $Id: AceBucket-3.1.lua 609 2008-03-26 08:48:54Z nevcairiel $ ]]
...
local MAJOR, MINOR = "AceBucket-3.1", 3
...
local function RegisterBucket(self, event, interval, callback, isMessage)
	...
	if not AceEvent or not AceTimer then 
		AceEvent = LibStub:GetLibrary("AceEvent-3.1", true)
		AceTimer = LibStub:GetLibrary("AceTimer-3.1", true)
		if not AceEvent or not AceTimer then
			error(MAJOR .. " requires AceEvent-3.1 and AceTimer-3.1", 3)
		end
	end
